...
NSString * TheMatterofMemory_Version_20$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation TheMatterofMemory_Version_20Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_thematterofmemory_version_20);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_thematterofmemory_version_20 gc];
}
...
{
	// create TheMatterofMemory_Version_20 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_thematterofmemory_version_20 = [[TheMatterofMemory_Version_20Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_thematterofmemory_version_20];
	
	NSString *_thematterofmemory_version_20NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _thematterofmemory_version_20NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_thematterofmemory_version_20 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:TheMatterofMemory_Version_20$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.thematterofmemory_version_20.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.thematterofmemory_version_20.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
