...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kTheMatterofMemory_Version_20UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kTheMatterofMemory_Version_20UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
